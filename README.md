...
新增独立应用 `store`，入口 `/store`，账号管理中的名称为“门店管理”。操作权限为 `coupon:view`、`coupon:issue`、`coupon:redeem`，发放和核销依赖查看。管理员、合伙人配置三店范围 `all`；店长按 `viewScope.stores` 显式选择门店，`ownership` 固定 `any`。角色不授予操作权限或账号管理权限。
