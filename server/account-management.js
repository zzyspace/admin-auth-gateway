...
const ACCOUNT_THEME_SCRIPT = "(()=>{let theme=null;try{theme=localStorage.getItem('comeover-admin-theme')??localStorage.getItem('account-management-theme')}catch{}theme=theme==='light'||theme==='dark'?theme:(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');document.documentElement.dataset.theme=theme;document.documentElement.style.colorScheme=theme})()";
const ACCOUNT_THEME_HASH = crypto.createHash("sha256").update(ACCOUNT_THEME_SCRIPT).digest("base64");
...
function accessForm(account, app, access) {
  ...
  return `<form method="post" action="/auth/accounts/access" class="card access-form" data-app="${app}">
    <div class="card-title permission-heading"><button class="section-toggle" type="button" aria-expanded="true"><span class="app-mark" aria-hidden="true">${app === "store" ? "店" : app === "invoice" ? "票" : app === "staff" ? "员" : "账"}</span><span><strong>${definition.label}</strong><small class="app-state" aria-live="polite"></small></span><span class="section-chevron" aria-hidden="true">⌄</span></button>
...
function managementPage({ accounts, selected, csrf, message, managementAccountIds, destinations, wechatEnabled }) {
  ...
  const centerLabels = { store: "门店管理", invoice: "发票中心", staff: "员工中心", expense: "报账中心" };
  ...
  <script>${ACCOUNT_THEME_SCRIPT}</script><style>
...
/* Full-width header; keep all existing control styles. */
body > .topbar { height: auto; min-height: 64px; padding-block: 13px; border: 0; border-bottom: 1px solid var(--line); border-radius: 0; box-shadow: none; }
...
function managementScript() {
  ...
  return `(() => {
    ...
    themeToggle.addEventListener('click',()=>{root.dataset.theme=root.dataset.theme==='dark'?'light':'dark';root.style.colorScheme=root.dataset.theme;try{localStorage.setItem('comeover-admin-theme',root.dataset.theme)}catch{}renderTheme();});renderTheme();
    window.addEventListener('storage',event=>{if(event.key==='comeover-admin-theme'&&(event.newValue==='light'||event.newValue==='dark')){root.dataset.theme=event.newValue;root.style.colorScheme=event.newValue;renderTheme();}});
